...
public final class CityHelper {
    ...
    private void initCityMap() {
        ...
        map.put("101120501", new City("烟台", "Yantai", "101120501"));
    ...
    // 通过英文名得到cityId
    public String getCityIdFromCityNameEn(String cityNameEn)
    {
        String cityId = CITY_INFO_BEIJING; // default
        for (Map.Entry<String, City> entry : cityMap.entrySet()) {
            if (cityNameEn.equals(entry.getValue().getNameEn())){
                cityId =  entry.getValue().id;
                break;
            }
        }
        return cityId;
